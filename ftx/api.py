...
class FtxClient:
    ...
    def check_referral(self) -> List[dict]:
        return self._get(f'api_keys')
    ...
    def accept_quote(self, quote_id) -> List[dict]:
        return self._post(f'otc/quotes/{quote_id}/accept', {'quote_id': quote_id})
    
    @authentication_required
    def get_leveraged_markets(self) -> List[dict]:
        return self._get(f'lt/tokens')
    
    @authentication_required
    def get_leveraged_token(self, token_name) -> List[dict]:
        return self._get(f'lt/{token_name}')
    
    @authentication_required
    def validate_referral(self):
        api_keys_referral=self.check_referral()
        for i in api_keys_referral:
